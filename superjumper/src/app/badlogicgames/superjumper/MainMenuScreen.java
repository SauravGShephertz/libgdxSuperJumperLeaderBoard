...
public class MainMenuScreen implements Screen {
	...
	public MainMenuScreen (Game game) {
		...
		registerBounds = new Rectangle(160 - 64, 60, 128, 32);
	...
	public void update () {
		if (Gdx.input.justTouched()) {
			...
			if (highscoresBounds.contains(touchPoint.x, touchPoint.y)) {
				Assets.playSound(Assets.clickSound);
				game.setScreen(new HighscoresScreen(game, 0));
				return;
			}
			...
			if (registerBounds.contains(touchPoint.x, touchPoint.y)) {
	...
	public void draw () {
		...
		batcher.draw(Assets.register, 160-64, 60, 128, 32);
