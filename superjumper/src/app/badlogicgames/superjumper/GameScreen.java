...
import app.badlogicgames.superjumper.World.WorldListener;
...
public class GameScreen implements Screen {
	...
	public GameScreen (Game game) {
		...
		submitBounds = new Rectangle(160 - 128 / 2, 100, 128, 32);
		lastScore = 0;
		scoreString = "SCORE: 0";
	}
	...
	private void updateGameOver () {
		if (Gdx.input.justTouched()) {
			guiCam.unproject(touchPoint.set(Gdx.input.getX(), Gdx.input.getY(), 0));
			if (submitBounds.contains(touchPoint.x, touchPoint.y)) {
				Assets.playSound(Assets.clickSound);
				if(RegisterScreen.isUserRegistered()){
					game.setScreen(new HighscoresScreen(game, lastScore));
				}else{
	...
	private void presentGameOver () {
		...
		batcher.draw(Assets.submit, 160 - 128 / 2, 100, 128, 32);
